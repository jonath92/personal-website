...
const RadioApplet = () => {
    ...
    function render1stParagraph() {
        return (
            <ProjectReport.P>
                I like listening to the radio as it allows you to discover new music and catch up with the latest news. While plenty of radio applications exist, I was never able to find an application which fully could meet my requirements. As a regular radio listener, I want to start or change a radio channel with as few clicks as possible. As far as I can see this requirement is best met with applications with a tray icon and a corresponding left-click menu. In Linux Mint (and other Linux Distributions which use Cinnamon as Desktop Environment) these types of software can easily be installed by so-called Applets.
            </ProjectReport.P>
        )
    }
    ...
    function render3thParagrah() {
        return (
            <ProjectReport.P>
                Using a player with MPRIS Support opens numerous opportunities to further extend the application. Thanks to <a href="https://en.wikipedia.org/wiki/D-Bus">D-Bus</a> and the <a href="https://gi.readthedocs.io/en/latest/goals.html">GObject introspection project</a> it is relatively easy to change the playback status, the volume and the position and also get informed when it has been changed remotely. Therefore I have already made some further additions such as adding a play/pause icon in front of the channel to indicate if a channel is playing or paused and an option to copy the title of the current song to the clipboard. After some contributions the creator of the Applet asked me to replace him as maintainer as he wants to focus on other projects.
            </ProjectReport.P>
        )
    }
